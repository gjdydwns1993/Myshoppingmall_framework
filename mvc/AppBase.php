...
abstract class AppBase{//계승을 전제로 하는 클래스로 abstract클래스로 정의
  ...
  //내부에서 수행하여 ActionMethod를 실행
  //Response객체에 저장되는 Rendeing되는 Data를 응답으로 Client로 전송
  public function run(){
    try {
      //ex)
      //url : http://weblog.localhost/index_error.php/
      //$parameters : Array ( [controller] => blog [action] => index [0] => / )
      //url : http://weblog.localhost/index_error.php/account/signup
      //$parameters : Array ( [controller] => account [0] => /account/signup
      //                      [action] => signup [1] => signup )
      ...
      //해당 controller객체생성후 dispatch를 통한 actionMethod실행
  ...
  public function getContent($controllerName,$action,$parameters = array()){
    ...
    //$this->getControllerObject 의 결과값이 false인경우
    if($controller === false){
      throw new FileNotFoundException($controllerClass.'NOT FOUND.');
    }
    //$content = 해당컨트롤러의 dispatch한 후 반환되는 결과값
  ...
  protected function getControllerObject($controllerClass) {
    ...
    if(!class_exists($controllerClass)) {
      ...
      if(!is_readable($controllerFile)) {
        ...
        //$controllerFile
        //ex ) C:\php\xampp\htdocs\weblog.localhost/controllers/BlogController.php
