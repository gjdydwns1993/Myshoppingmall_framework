<?php
  abstract class Controller { //추상 클래스 이므로 사용하려면 계승해야함
    ...
    protected function render($param = array(),$viewFile = null, $template = null){
      ...
      //$info(request객체,base_url,session객체)
      $view = new View($this->_application->getViewDirectory(),$info);

      if(is_null($viewFile)){
        $viewFile = $this->_action;//action명
      }
      if(is_null($template)){
        $template = 'template';
      }
      $path = $this ->_controller.'/'.$viewFile; //뷰파일의 경로 정보
      $contents = $view -> render($path, $param, $template); //view 클래스의 render()
      return $contents;
    }
    ...
    protected function redirect($url){
      ...
      $this->_response->setStatusCode(302,'FOUND'); //상태코드 302 리다이렉트
    ...
    protected function getToken($form){
      ...
      //$tokens = $this->_session->get($key, array()); //$_SESSION변수에 Token을 저장하기 위한 키 이름 작성추가하셔야 됩니다
